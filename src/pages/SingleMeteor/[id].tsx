...
const SingleMeteor = () => {
  ...
  async function getData() {
    fetch(
      `https://api.nasa.gov/neo/rest/v1/neo/${router.query.id}?api_key=w72K7LgVIHRXzd4b8VfGskoRL1FjgdQPPlhA5vGg`
    )
      ...
      .then((data) => {
        ...
        for (let tmp of data.close_approach_data) {
          let approach: dataApproach = {
            ...
            distance: Math.round(tmp.miss_distance.kilometers).toLocaleString(
              "ru"
            ),
            velocity: Math.round(
              tmp.relative_velocity.kilometers_per_hour
            ).toLocaleString("ru"),
            ...
            <li className={styles.li}>
              <Table hideHeader>
                ...
                <TableBody>
                  <TableRow key="1">
                    <TableCell>
                      <b>Дата максимального приближения: </b>
                    </TableCell>
                    <TableCell>{app.date}</TableCell>
                  </TableRow>
                  <TableRow key="2">
                    <TableCell>
                      <b>Расстояние до Земли:</b>
                    </TableCell>
                    <TableCell>{app.distance} км</TableCell>
                  </TableRow>
                  <TableRow key="3">
                    <TableCell>
                      <b>Скорость относительно Земли:</b>
                    </TableCell>
                    <TableCell>{app.velocity} км/ч</TableCell>
                  </TableRow>
                  <TableRow key="4">
                    <TableCell>
                      <b>По орбите вокруг</b>
                    </TableCell>
